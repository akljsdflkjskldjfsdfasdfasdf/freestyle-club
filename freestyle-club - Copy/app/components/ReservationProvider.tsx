...
export default function ReservationProvider({
  ...
  useEffect(() => {
    ...
    // Fokusiraj prvo polje samo na desktopu (miš), bez skrola — da na mobilnom
    // ne iskače tastatura i da X ostane vidljiv pri otvaranju.
    const t = setTimeout(() => {
  ...
  return (
    <ReservationContext.Provider value={{ open }}>
      ...
      {isOpen && activeType && (
        <div
          className="animate-fade-in fixed inset-0 z-[70] flex items-end justify-center bg-ink/80 backdrop-blur-sm sm:items-center sm:p-6"
          onClick={close}
          role="dialog"
          aria-modal="true"
          aria-labelledby="rez-naslov"
        >
          {/* Panel je ograničen na visinu ekrana; X je uvek vidljiv (van skrola),
              a telo forme skroluje unutar panela. */}
          <div
            className="animate-pop-in panel relative flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border-white/10 sm:max-h-[88dvh] sm:rounded-3xl"
          ...
          >
            <button
              ...
              onClick={close}
              aria-label="Zatvori"
              className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-ink/70 text-white/80 backdrop-blur-sm transition-colors hover:bg-white/10 hover:text-white"
            >
              ✕
            </button>

            {status === "success" ? (
              <div className="overflow-y-auto p-6 py-8 text-center sm:p-8">
              ...
              >
                <p
                  id="rez-naslov"
                  className="pr-10 font-display text-xl font-extrabold text-white"
                >
