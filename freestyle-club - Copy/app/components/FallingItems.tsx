  ...
  useGSAP(
    () => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        // Padaj kroz celu visinu hero sekcije (ne samo viewport-a) — inače
        // stvari nestaju u vazduhu kad je hero viši od ekrana (mobilni).
        // VAŽNO: visinu keširamo (i osvežavamo na resize), jer čitanje
        // offsetHeight unutar GSAP vrednosti bi pravilo reflow na svaki ciklus
        // → trzanje (lag) svih animacija u hero-u.
        const measure = () =>
          (scope.current?.offsetHeight ?? window.innerHeight) + 260;
        let fallDistance = measure();
        const ro = new ResizeObserver(() => {
          fallDistance = measure();
        });
        if (scope.current) ro.observe(scope.current);

        const fallTo = () => fallDistance;
        const anims: gsap.core.Animation[] = [];

        items.forEach((el) => {
          const dur = gsap.utils.random(6, 10);

          // Jedna samostalna petlja po stvari: sve (pad, zanošenje, rotacija,
          // pojavljivanje/nestajanje) je u istom timeline-u, pa svaki ciklus
          // počinje čisto, bez desinhronizacije.
          const tl = gsap
            .timeline({
              repeat: -1,
            ...
            .to(el, { autoAlpha: 0.85, duration: 0.8 }, 0)
            .to(
              el,
              {
                y: fallTo,
                x: "+=" + gsap.utils.random(-70, 70),
                rotation: "+=" + gsap.utils.random(180, 480),
                duration: dur,
                ease: "none",
              },
              0,
            )
            .to(el, { autoAlpha: 0, duration: 1 }, dur - 1);
          anims.push(tl);
        });
